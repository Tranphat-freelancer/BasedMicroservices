...
abp new "$name.AdminService" -t module --no-ui -o services\administration
...
abp new "$name" -t app -u mvc -m none --separate-identity-server --database-provider ef -csf -o temp
Move-Item -Path ".\temp\$name\mvc\" -Destination .\apps\mvc
...
dotnet sln ".\$name.sln" remove (Get-ChildItem -r **/*.AdminService.IdentityServer.csproj)
...
Remove-Item -Recurse -Force (Get-ChildItem -r **/*.AdminService.IdentityServer)
...
abp add-module Volo.AuditLogging -s "services\administration\$name.AdminService.sln" --skip-db-migrations
abp add-module Volo.FeatureManagement -s "services\administration\$name.AdminService.sln" --skip-db-migrations
abp add-module Volo.PermissionManagement -s "services\administration\$name.AdminService.sln" --skip-db-migrations
abp add-module Volo.SettingManagement -s "services\administration\$name.AdminService.sln" --skip-db-migrations
